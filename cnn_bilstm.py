...
from tensorflow.keras.layers import Dense, Activation, Conv1D, MaxPooling1D, GlobalAveragePooling1D, Flatten, Dropout, BatchNormalization, Input, LSTM, Bidirectional
...
for i in range(0,55):
    ...
    x1 = Bidirectional(LSTM(256,activation='tanh'))(x1)
    x1 = Dropout(0.5)(x1)
...
df.to_csv('results/final_results/Accuracies_1dcnn_bilstm.csv',index=False)
np.save('results/final_results/pred_arr_1dcnn_bilstm.npy',pred_arr)
np.save('results/final_results/test_arr_1dcnn_bilstm.npy',test_arr)
